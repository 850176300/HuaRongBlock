...
//  SvUDID
//
//  Created by 刘 波 on 13-11-29.
//  Copyright (c) 2013年 maple. All rights reserved.
//
...
static const char kKeychainUDIDItemIdentifier[]  = "UUID";
static const char kKeyChainUDIDAccessGroup[] = "L76KY7YU4W.com.kekestudio.LawLibrary";

@implementation KKUDID

+ (NSString*)UDID{
    NSString *udid = [KKUDID getUDIDFromKeyChain];
    if (!udid) {
        ...
        if (version >= 7.0) {
            udid = [KKUDID _UDID_iOS7];
        }
        else if (version >= 2.0) {
            udid = [KKUDID _UDID_iOS6];
        }
        
        [KKUDID setUDIDToKeyChain:udid];
    }
...
+ (BOOL)setUDIDToKeyChain:(NSString*)udid{
    ...
    if ([KKUDID getUDIDFromKeyChain]) {        // there is item in keychain
        [KKUDID updateUDIDInKeyChain:udid];
        [dictForAdd release];
        return YES;
    }
